...
public class Hechicero extends Casta {
     ...
     * será posible y se retornará false. El parámetro caster a su vez llama al
     * método calcularPuntosDeMagia() el cual luego se multiplica por 1.5
     *
     * @param caster
     *            Personaje que realiza la habilidad
     * @param atacado
     *            puede ser una instancia de Persona o NPC dependiendo de la misma,
    ...
    public final boolean habilidad1(final Personaje caster, final Peleable atacado) {
        ...
        if (resultado) {
            ...
            resultado = (atacado.serAtacado((int) (caster.calcularPuntosDeMagia() * MULTIPLICADORMAGIA)) > 0);
     ...
     * atacante(caster) posea 10 o más del atributo energia ya que estos se
     * descuentan seguido de comprobar que los posee, de lo contrario el ataque no
     * será posible y se retornará false. Para que éste método tenga posibilidad de
     * retornar true, aliado debe ser de la instancia Personaje
     *
     * @param caster
     *            Personaje que realiza la habilidad
     * @param aliado
     *            atacado puede ser una instancia de Persona o NPC dependiendo de la
     *            misma podrá o no retornar true el método.
     * @return booleano que determina si el ataque fue exitoso o no.
     */
    @Override
    public final boolean habilidad2(final Personaje caster, final Peleable aliado) {
        ...
        if (resultado) {
            caster.reducirEnergia(ENERGIAMINIMA);
            resultado = (aliado instanceof Personaje);
            if (resultado) {
     ...
     * descuentan seguido de comprobar que los posee, de lo contrario el ataque no
     * será posible y se retornará false. Para que éste método tenga posibilidad de
     * retornar true, el Atacado debe ser una instancia Personaje
     *
     * @param caster
     *            Personaje que realiza la habilidad
     * @param atacado
     *            El atacado puede ser una instancia de Persona o NPC dependiendo de
     *            la misma podrá o no retornar true el método.
    ...
    public final boolean habilidad3(final Personaje caster, final Peleable atacado) {
        if (caster.getEnergia() >= ENERGIAMINIMA) {
            ...
            if (atacado instanceof Personaje) {
                final int energiaRobada = ((Personaje) atacado).serDesernegizado(caster.calcularPuntosDeMagia());
                final int saludRobada = ((Personaje) atacado)
