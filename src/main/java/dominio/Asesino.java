...
public class Asesino extends Casta {
     ...
     * inicializando probabilidadGolpeCritico, probabilidadEvitarDanio,
     * danioCritico, con valores predeterminados.
     */
    public Asesino() {
        super();
    }
     ...
     * ataque. La primera condición para que el ataque pueda realizarse es que el
     * atacante(caster) posea 10 o más del atributo energia ya que estos se
     * descuentan seguido de comprobar que los posee, de lo contrario el ataque no
     * será posible y se retornará false De ser posible el ataque, se llama al
     * método serAtacado() del argumento atacado el cual actuará de cierta manera
     * dependiendo de que clase sea el argumento atacado
     *
     * @param caster
     *            Personaje atacante
     * @param atacado
     *            puede recibir como argumento una instancia de Personaje o de NPC
     *            (NonPlayableCharacter) ya que ambas implementan la interface
     *            Peleable
    ...
    public final boolean habilidad1(final Personaje caster, final Peleable atacado) {
        ...
        boolean resultado = (caster.getEnergia() >= ENERGIAMINIMA);

        if (resultado) {
            caster.reducirEnergia(ENERGIAMINIMA);
            resultado = (atacado.serAtacado((int) (caster.getAtaque() * caster.getCasta().getDanioCritico())) > 0);
        }
     ...
     * atacante(caster) posea 10 o más del atributo energia ya que estos se
     * descuentan seguido de comprobar que los posee, de lo contrario el ataque no
     * será posible y se retornará false
     *
     * @param caster
     *            Personaje que realiza el ataque, en este caso no es un ataque si
     *            no un aumento en las caracteristicas del caster en este caso
    ...
    public final boolean habilidad2(final Personaje caster, final Peleable atacado) {
        final boolean resultado = (caster.getEnergia() >= ENERGIAMINIMA);
        if (resultado) {

            caster.reducirEnergia(ENERGIAMINIMA);
            final double limite = 0.5;
            if (this.getProbabilidadEvitarDanio() + AUMENTARPROBEVITAR < limite) {
    ...
    /**
     * Retorna un entero. Que otorga el bonus de inteligencia perteneciente a esta
     * casta.
     *
     * @return Retorna 0 para esta casta.
     */
