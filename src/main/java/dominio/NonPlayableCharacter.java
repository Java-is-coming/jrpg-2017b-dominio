...
public class NonPlayableCharacter extends MadreDeTodo implements Peleable {
     ...
     * parámetro al constructor, aumentará o disminuirá el valor de los atributos
     * fuerza salud y defensa.
     *
     * @param nombre
     *            Nombre que se le otorga al NPC
     * @param nivel
     *            Nivel que se le otorga al NPC
     * @param dificultadNPC
     ...
     * Personaje que produjo la disminución de la salud del NPC a 0. La misma sera
     * MULTIPLCADOREXPNPC veces el valor del atributo nivel
     *
    ...
    /**
     * Retorna un booleano. Que indica si el NPC esta vivo, evaluando si el mismo
     * tiene salud mayor a 0.
     *
    ...
    /**
     * Método que, dependiendo de MyRandom.nextdouble() y NUMEROPARAATACAR. Puede
     * ejecutar un ataque mejorado por el atributo MULTIPLICADORFUERZA
     *
     * @param atacado
    ...
    @Override
    /**
     * Dependiendo de MyRandom.nextdouble() y NUMEROPARASERATACADO. Puede disminuir
     * el daño dependiendo del atributo DIVISORDEDEFENSA.
     *
     * @param danio
    ...
    public final int serAtacado(final int danio) {
        ...
        if (this.getRandom().nextDouble() >= NUMEROPARASERATACADO) {
            danioCalc -= this.getDefensa() / DIVISORDEDEFENSA;
            if (danioCalc > 0) {
                salud -= danioCalc;
                return danioCalc;
            }
