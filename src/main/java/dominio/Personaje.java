...
public abstract class Personaje extends MadreDeTodo implements Peleable, Serializable {
    ...
    /**
     * Cantidad a multiplicar. Para obtener la experiencia otorgada por el personaje
    ...
    /**
     * Metodo que retorna las habilidades que posee el personaje. Segun la casta del
     * mismo.
     *
    ...
    /**
     * Metodo estatico. Que sirve para cargar la tabla de niveles de personaje.
     */
     ...
     * completados por las clases hijo (Elfo,Humano,Orco) como por ejemplo el array
     * habilidadesRaza[]. Dependiendo de qué instancia es el parámetro casta, se
     * incrementará en 5 un atributo del personaje
     *
     * @param nombre
     *            Indica el nombre el personaje
     * @param casta
     *            Indica la casta(Raza) del personaje y con ella el incremento que
     *            tendrá cierto atributo
     * @param id
     *            Identificador del personaje
     */
     ...
     * del juego. Algunos serán completados por las clases hijo (Elfo,Humano,Orco)
     * como por ejemplo: El array habilidadesRaza[], a diferencia del constructor de
     * sólo 3 parámetros, éste recibe la mayoría de los atributos.
     *
     * @param nombre
    ...
    /**
     * Metodo void que sobreescribe el atributo de ataque con el parametro enviado.
     *
     * @param ataque
    ...
    /**
     * Metodo void que sobreescribe el atributo clan. Aniade al personaje llamador
     * al clan enviado como parámetro.
     *
     * @param clan
     *            Nueva del personaje.
     */
    ...
    /**
     * Método que retorna un entero. Dpendiendo del resultado de las comparaciones
     * entre el Personaje llamador y el argumento que puede ser instancia de
     * Personaje o de NPC (NonPlayableCharacter) La probabilidad de golpe critico
     * depende de la casta del Personaje y de la destreza del mismo. Si la
     * probabilidad junto con la destreza es mayor o igual al número generado de
     * manera aleatoria entonces se atacará con golpe crítico, de lo contrario sera
     * atacado con el valor del atributo ataque.
     *
     * @param atacado
    ...
    /**
     * Metodo que retorna un entero. Que depende a que casta pertenece el personaje
     * y que ataque poseaa. El danio critico se obtiene de la clase casta. El entero
     * surge de la multiplicacion del ataque del personaje y el danio critico de la
     * casta que pertenece.
     *
     ...
     * realizara el personaje. Estos puntos dependen de la fuerza del personaje y
     * MULTIPLICADORFZA. MULTIPLICADORFZA atributo static de la clase Personaje.
     *
    ...
    /**
     * Metodo void que establece la energia del personaje como la maxima posible que
     * puede tener.
     */
     ...
     * MULTIPLICADORFZA(constante). Defensa depende de la destreza. Magia depende de
     * la inteligencia y de MULTIPLICADORMGA (constante).
     */
    ...
    /**
     * Metodo que retorna boolean heredado de la interface Peleable. Si la salud del
     * personaje es mayor a 0 este está vivo.
     *
     ...
     * dependiendo del resultado de las comparaciones, si el número generado con la
     * clase MyRandom es mayor a la probabilidad de evitar danio, La cual depende de
     * la casta del Personaje, entonces no podrá evitarse el ataque, se descontará
     * el valor del argumento danio al atributo salud. Si el valor del atributo
     * salud es menor al valor del argumento danio, se procederá a igualar el
     * atributo salud a 0 y retornar el danio realziado (que será igual a la salud
     * antes de que esté en 0)
     *
     * @param danio
    ...
    public final int serAtacado(final int danio) {
        int danioCalculo = danio;
        if (this.getRandom().nextDouble() >= this.getCasta().getProbabilidadEvitarDanio()) {
            danioCalculo -= this.getDefensa();
            if (danioCalculo > 0) {
                if (salud <= danioCalculo) {
                    danioCalculo = salud;
                    salud = 0;
                } else {
                    salud -= danioCalculo;
    ...
    /**
     * Metodo que retorna un entero de los puntos de salud quitados al personaje. Al
     * danio total recibido se le resta la defensa del personaje. Si este danio es
     * menor o igual a la salud del personaje se le quita danio puntos de salud. Si
     * el danio es mayor a la salud se establece la salud del personaje en 0.
     *
     * @param danio
     *            Danio causado al personaje
     * @return Retorna los puntos de vida quitados al personaje
     */
    public final int serRobadoSalud(final int danio) {
        int danioCalculo = danio;
        danioCalculo -= this.getDefensa();
        if (danioCalculo <= 0) {
            return 0;
        }
        if ((salud - danioCalculo) >= 0) {
            salud -= danioCalculo;
        } else {
            danioCalculo = salud;
            salud = 0;
    ...
    /**
     * Metodo que retorna los puntos de energia quitados al personaje. Al danio
     * total ejercido al personaje se le resta la defensa del mismo. Si este danio
     * es menor o igual a la energia del personaje se le resta al mismo, sino se
     * establece energia como 0.
     *
     * @param danio
     *            Danio causado al personaje
     * @return Retorna los puntos de energia quitados al personaje.
     */
    public final int serDesernegizado(final int danio) {
        int danioCalculo = danio;
        danioCalculo -= this.getDefensa();
        if (danioCalculo <= 0) {
            return 0;
        }
        if ((energia - danioCalculo) >= 0) {
            energia -= danioCalculo;
        } else {
            danioCalculo = energia;
            energia = 0;
        }
        return danioCalculo;
    }
     ...
     */
    public final void serCurado(final int saludSumar) {
        if ((this.salud + saludSumar) <= this.saludTope) {
    ...
    /**
     * Metodo void que aumenta la energia actual del personaje. Si este aumento es
     * mayor al tope establece como energia actual la maxima que puede tener el
     * personaje.
     *
     * @param energiaSumar
     *            Puntos de energia a sumar al Personaje.
     */
    public final void serEnergizado(final int energiaSumar) {
        if ((this.energia + energiaSumar) <= this.energiaTope) {
            this.energia += energiaSumar;
        } else {
            this.energia = this.energiaTope;
        }
    }
     ...
     * personaje y lo aniade a la lista de Personajes que integran la alianza.
     *
     * @param nombreAlianza
    ...
    /**
     * Metodo que retorna un boolean si pudo aniadir un nuevo aliado a la alianza en
     * la que se encuentra el personaje. Si el personaje llamador no pertenece a
     * ninguna alianza se crea una con el nombre "Alianza 1" por defecto. Luego se
     * agrega al nuevo aliado enviado como parametro a la alianza del personaje
     * llamador.
     *
     * @param nuevoAliado
     *            Personaje que se aniadira al clan del llamador
     ...
     * inteligencia del personaje. Nunca estos atributos superan los maximos. Una
     * vez modificados los atributos mencionados actualiza los nuevos puntos de
     * ataque,defensa y magia del personaje.
     *
     * @param fuerza
     *            Nueva fuerza del personaje
     * @param nuevaDestreza
     *            Nueva destreza del personaje
     * @param nuevaInteligencia
     *            Nueva inteligencia del personaje
     */
    public final void AsignarPuntosSkills(final int fuerza, final int nuevaDestreza, final int nuevaInteligencia) {
        if (this.getFuerza() + fuerza <= FUERZAMAXIMA) {
            this.aumentarFuerza(fuerza);
        }
        if (this.destreza + nuevaDestreza <= DEFENSAMAXIMA) {
            this.destreza += nuevaDestreza;
        }
        if (this.inteligencia + nuevaInteligencia <= INTELIGENCIAMAXIMA) {
    ...
    /**
     * Metodo void que aumenta el nivel del personaje Si ya se encuentra en el nivel
     * maximo no realiza cambios. Pero si no se alcanzo al nivel maximo se
     * actualizara la salud tope, la energia tope y el nivel del personaje hasta que
     * su experiencia sea menor a la de un nivel preestablecido. Luego se le
     * descuenta al atributo experiencia la experiencia que se utilizo para aumentar
     ...
     * cantidad suficiente de experiencia para aumentar el nivel o no. Si retorna
     * true aumento de nivel y false no.
     *
     * @param exp
    ...
    /**
     * Metodo que retorna un entero con la experiencia equivalente del personaje que
     * depende del nivel del mismo y de MULTIPLICADOREXP. MULTIPLICADOREXP atributo
     * estatico de la clase personaje.
     *
     ...
     * personaje Asesino, Guerrero o Hechicero. La energia del personaje debe ser
     * mayor a la minima para lograr la habilidad.
     *
     * @param atacado
     ...
     * Hechicero al que pertenece el personaje. La energia del personaje debe ser
     * mayor a la minima para lograr la habilidad.
     *
     * @param atacado
     *            Es el personaje al cual le realizará la habilidad el personaje
     ...
     * la habilidad. Esta habilidad dependerá de la raza al que pertenece el
     * personaje Humano, Orco o Elfo. La energia del personaje debe ser mayor a la
     * minima para lograr la habilidad, independientemente de la raza que sea.
     *
     * @param atacado
     *            Es el personaje al cual le realizará la habilidad el personaje
     *            llamador.
    ...
    /**
     * Metodo abstracto implementado en cada raza. Que retorna un vector String con
     * los nombres de las habilidades de esa raza. Depende de la raza que sea el
     * personaje llamador, Humano, Orco o Elfo.
     *
     * @param atacado
     *            Es el personaje al cual le realizará la habilidad el personaje
     *            llamador.
     ...
     * bonus de salud. Depende de la raza que sea el personaje llamador Humano, Orco
     * o Elfo.
     *
    ...
    /**
     * Metodo abstracto implementado en cada raza. Que retorna un entero con el
     * bonus de energia. Depende de la raza que sea el personaje llamador, Humano,
     * Orco o Elfo.
     *
     ...
     * nombre de la raza que pertenece el personaje llamador Depende de la raza que
    ...
    public void trueque(final ArrayList<Item> misItems, final ArrayList<Item> aPoner,
        ...
        int j = 0;
        boolean loop = true;
        final ArrayList<Item> aux = misItems;
        while (aSacar.size() > 0) {
            while (loop) {
