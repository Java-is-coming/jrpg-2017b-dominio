...
public class Guerrero extends Casta {
    ...
    public final boolean habilidad1(final Personaje caster, final Peleable atacado) {
        ...
        if (resultado) {
            ...
            resultado = (atacado.serAtacado(caster.getAtaque() * MULTIPLICADORFUERZA) > 0);
     ...
     * atacante(caster) posea 10 o más del atributo energia ya que estos se
     * descuentan seguido de comprobar que los posee, de lo contrario el ataque no
     * será posible y se retornará false. Éste método sólo retornará true si el
     * argumento atacado, es una instancia de Personaje.
     *
     * @param caster
     *            Personaje atacante
     * @param atacado
     *            Éste argumento debe de ser clase atacado para que el método tenga
     *            posibilidad de retornar true
     * @return retorna si se ejecutó correctamente la habilidad
     */
    @Override
    public final boolean habilidad3(final Personaje caster, final Peleable atacado) {
        if (caster.getEnergia() >= ENERGIAMINIMA) {
            ...
            if (atacado instanceof Personaje) {
                final int defensaOriginal = ((Personaje) atacado).getDefensa();
